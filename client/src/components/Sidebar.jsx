...
// FIXED: Updated Sidebar component to properly use the onLogout prop
const Sidebar = ({ currentUser, onLogout }) => {
  ...
  const navigationItems = useMemo(() => {
    if (currentUser?.role === "Admintanod" || currentUser?.role === "Tanod") {
      return [
        { 
    ...
    } else if (currentUser?.role === "Admin Tignoan") {
      return [
        { 
          path: '/dashboard', 
          label: 'Dashboard',
  ...
  // FIXED: Updated handleLogout to use the onLogout prop properly
  const handleLogout = () => {
    console.log('Sidebar logout initiated...');
    
    // Immediately close dropdown and set loading
    setShowProfileDropdown(false);
    setIsLoading(true);
    
    // Call the parent component's logout function immediately
    if (onLogout) {
      console.log('Calling parent logout function...');
      onLogout();
    } else {
          ...
          <div className="user-avatar-container">
            ...
            {showProfileDropdown && (
              <div className={`profile-dropdown ${isCollapsed ? 'collapsed' : ''}`}>
                ...
                {/* FIXED: Changed from Link to button and properly call handleLogout */}
